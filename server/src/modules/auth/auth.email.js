...
const buildVerificationMessage = (token, user) => {
  const verificationUrl = `${env.frontendUrl || 'http://localhost:3000'}/verify-email?token=${encodeURIComponent(token)}`;
...
<div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; line-height: 1.6; color: #1e293b; background-color: #f8fafc; padding: 40px 20px;">
  <div style="max-width: 540px; margin: 0 auto; background-color: #ffffff; border-radius: 16px; padding: 40px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1); border: 1px solid #e2e8f0;">
    ...
    <p style="font-size: 15px; color: #475569; margin-bottom: 24px; text-align: center; line-height: 1.6;">
      Thank you for signing up for Autonomous Study Planner. To start planning your adaptive study journey, please verify your email address.
    </p>
    ...
    <div style="margin-top: 40px; text-align: center; font-size: 12px; color: #94a3b8;">
      Need help? Contact our support at <a href="mailto:support@studypilot.local" style="color: #94a3b8; text-decoration: underline;">support@studypilot.local</a>.
    </div>
  </div>
</div>
`.trim();
...
const buildPasswordResetMessage = (token, email) => {
  const resetUrl = `${env.frontendUrl}/reset-password?token=${encodeURIComponent(token)}&email=${encodeURIComponent(email)}`;

  return {
    subject: 'Reset your Autonomous Study Planner password',
    text: `Reset token: ${token}. Suggested link: ${resetUrl}`,
    html: `<p>Your password reset token is <strong>${token}</strong></p><p>Suggested link: <a href="${resetUrl}">${resetUrl}</a></p>`,
  };
};
